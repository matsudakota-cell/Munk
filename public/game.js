import {NEIGHBORS,activityHint,activityMissions} from './island-activities.mjs';
import {buildNeighborhoods} from './island-scene.mjs';
...
import {mimicTarget} from './mimicry.mjs';
...
for(let i=0;i<55;i++){let x=-46+rand()*92,z=-43+rand()*86;const clear=NEIGHBORS.some(a=>Math.hypot(x-a.x,z-a.z)<12)||ZONES.some(a=>Math.hypot(x-a.x,z-a.z)<14)||Math.abs(x)<6||Math.hypot(x-12,z-35)<8||Math.hypot(x-NEST.x,z-NEST.z)<8||FINDS.some(o=>Math.hypot(x-o.x,z-o.z)<3);if(!clear)tree(x,z,5+rand()*4,i%2)}for(let i=0;i<23;i++){let x=-47+i*4.2;tree(x,-44,5+rand()*3,i%2);if(i%2===0)tree(x,44,5+rand()*2,1)}
for(let i=0;i<90;i++){let x=-46+rand()*92,z=-43+rand()*86;if(NEIGHBORS.some(a=>Math.hypot(x-a.x,z-a.z)<12)||ZONES.some(a=>Math.hypot(x-a.x,z-a.z)<12)||Math.abs(x)<5)continue;const g=group(x,.1,z);for(let j=0;j<3;j++){let leaf=mesh(new THREE.ConeGeometry(.15,.7,3),'#6f9c52',j*.19,.2,0,[1,1,1],g);leaf.rotation.z=(j-1)*.35}if(i%4===0){cyl('#799855',0,.4,0,.045,.8,g);ball(i%3?'#f4da78':'#e4a29a',0,.83,0,[.25,.16,.25],g)}}
const neighborhoods=buildNeighborhoods({group,box,ball,cyl,tube,textSprite,terrainDisc,path,mat});
state.solid.push({x:-29,z:-5,r:1.4,h:2.6},{x:33,z:-5,r:2.2,h:1.7});
...
function makeFind(id){
 ...
 }else if(id==='watering-can'){
  cyl('#74bbc0',0,.4,0,.43,.7,g);tube([[.3,.3,0],[.8,.5,0],[1,.8,0]],.13,'#74bbc0',g);tube([[-.3,.15,0],[-.8,.5,0],[-.3,.85,0]],.09,'#dbe4b0',g);
 }else if(id==='fruit-basket'){
  box('#bc925d',0,.3,0,[1.2,.5,.85],g);tube([[-.55,.5,0],[0,1.2,0],[.55,.5,0]],.07,'#d9b775',g);for(let k=0;k<4;k++)ball(k%2?'#f7d76c':'#ed9b56',(k%2-.5)*.5,.65,(Math.floor(k/2)-.5)*.4,[.24,.25,.24],g);
 }else{
...
const playDisplays=NEIGHBORS.map(n=>makeFind(n.item));
function animateNest(){
 state.finds.forEach((item,i)=>{
  ...
  if(item.place==='held'){
   ...
   const areaIndex=NEIGHBORS.findIndex(n=>n.item===item.id);
   const flight=areaIndex<0?null:state.activities.areas[areaIndex].flight;
   let at=item.place==='nest'?nestDisplay(item.id):item;
   if(flight){const t=Math.min(1,(flight.age+state.activities.clock)/.75);at={x:flight.start.x+(flight.end.x-flight.start.x)*t,z:flight.start.z+(flight.end.z-flight.start.z)*t,y:1.2+Math.sin(t*Math.PI)*2};}
   g.position.set(at.x,at.y+.12,at.z);g.rotation.set(0,i*.6,0);g.scale.setScalar(1);
  }
 });
 playDisplays.forEach((g,j)=>{g.visible=state.nest.deposited.includes(NEIGHBORS[j].item);const at=nestDisplay(NEIGHBORS[j].item);g.position.set(at.x,at.y,at.z);});
...
function handleOokEvent(event){
 if(event.type==='mimic'){sound(event.player?420:550,.12);sound(event.player?500:660,.14,.18);return true;}
...
function actionHint(p,i){const key=i?'↵':'E';const activity=activityHint(state,i);if(activity)return `${key} · ${activity}`;if(LADDERS.some(l=>dist(p,l)<3))return `${key} · ${p.y>3?'Climb down':'Climb into the canopy'}`;if(p.y>6&&[-35,-23].some(x=>dist(p,{x,z:-30})<3.5))return `${key} · Ring your bell — partner rings the other`;const d=DRUMS.find(d=>dist(p,d)<2.7);if(d)return `${key} · Play ${d.note}  |  Next: ${state.songComplete?'Encore!':DRUMS[SONG[state.band]].note}`;if(state.balls.some(b=>dist(p,b)<3.4))return `${key} · Roll this coconut`;if(PADS.some(t=>dist(p,t)<2))return 'Stay on your pedal. Your partner takes the other!';if(TRAMPOLINES.some(t=>dist(p,t)<3))return 'Bounce up through the golden hoop!';if(nestLadderAction(p))return `${key} · ${insideNest(p)?'Climb down':'Climb home'}`;const find=findAction(state,p);if(find)return `${key} · ${find.kind==='take'?'Pick up '+find.item.name:'Put it down'}`;const hat=hatAction(state.fisherman,p,i);if(hat)return `${key} · ${hat==='take'?'Try on the hat':hat==='return'?'Put the hat back':'Put the hat down'}`;return ''}
function contextHint(p,i){const action=actionHint(p,i),copy=mimicTarget(state,i);return [action,copy?`${i?'/':'Q'} · Copy ${copy.kind}`:''].filter(Boolean).join('   ·   ')}
let lastUI='';function refresh(){const ms=missions(state),nearest=nearZone(state.players[0]);const extras=activityMissions(state);const signature=JSON.stringify([...ms,...extras].map(m=>[m.done,m.progress]))+nearest.id;if(signature!==lastUI){lastUI=signature;$('missionList').innerHTML=ms.map((m,i)=>`<div class="mission ${m.done?'done':m.zone.id===nearest.id?'active':''}"><div class="num">${m.done?'✓':i+1}</div><div><b>${m.short}</b><small>${m.zone.name} · ${m.progress}</small></div></div>`).join('');$('score').textContent=`${ms.filter(m=>m.done).length} / 5`;$('activityList').innerHTML=extras.map(m=>`<div class="mission ${m.done?'done':''}"><div class="num">${m.done?'✓':m.icon}</div><div><b>${m.short}</b><small>${m.zone.name} · ${m.done?'Made some monkey magic!':m.progress}</small></div></div>`).join('')}$('hint0').textContent=contextHint(state.players[0],0);$('hint1').textContent=contextHint(state.players[1],1);$('location').innerHTML=split?'Off on your own adventures<span>A WHOLE ISLAND TO EXPLORE</span>':`${nearest.name}<span>MONKEY ISLAND</span>`;$('location').style.opacity=($('hint0').textContent||$('hint1').textContent)?'0':'1'}
function drawMap(){const c=$('map').getContext('2d');c.clearRect(0,0,640,530);c.fillStyle='#92c5ba';c.beginPath();c.roundRect(0,0,640,530,28);c.fill();c.fillStyle='#abc580';c.beginPath();c.roundRect(30,25,580,480,55);c.fill();const pt=(x,z)=>[320+x*5.4,260+z*5];c.strokeStyle='#e6d5a7';c.lineWidth=15;c.lineCap='round';ZONES.forEach(z=>{c.beginPath();c.moveTo(320,300);c.lineTo(...pt(z.x,z.z));c.stroke()});missions(state).forEach((m,i)=>{const [x,z]=pt(m.zone.x,m.zone.z);c.fillStyle=m.done?'#4e855f':m.zone.color;c.beginPath();c.arc(x,z,27,0,7);c.fill();c.fillStyle='#304f3c';c.textAlign='center';c.font='bold 20px system-ui';c.fillText(m.done?'✓':String(i+1),x,z+7);c.font='bold 14px system-ui';c.fillText(m.zone.name,x,z+49)});NEIGHBORS.forEach((n,j)=>{const [x,z]=pt(n.x,n.z);c.fillStyle=n.color;c.beginPath();c.arc(x,z,23,0,7);c.fill();c.fillStyle='#304f3c';c.font='bold 22px system-ui';c.fillText(j?'☂':'❀',x,z+6);c.font='bold 14px system-ui';c.fillText(j?'Fruit stand':'Garden',x,z+42);});const [nx,nz]=pt(NEST.x,NEST.z);c.fillStyle='#fff0c8';c.font='bold 28px system-ui';c.fillText('⌂',nx,nz);c.font='bold 12px system-ui';c.fillText('Nest',nx,nz+18);state.players.forEach((p,i)=>{const [x,z]=pt(p.x,p.z);c.fillStyle=i?'#75bedb':'#f5c94f';c.strokeStyle='#fff8dc';c.lineWidth=3;c.beginPath();c.arc(x,z,9,0,7);c.fill();c.stroke();c.fillStyle='#304c3d';c.font='bold 12px system-ui';c.fillText(i?'Momo':'Pip',x,z-16)})}
...
state.players.forEach((p,i)=>{
 ...
 m.arms.forEach((arm,j)=>{
  ...
  else if(state.activities.areas.some(a=>a.holder===i)){arm.rotation.x=-1.5;arm.rotation.z=j?-.35:.35}
...
animateFisherman();animateNest();neighborhoods.animate(state);
...
renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();pause();toast('The 3D view paused. Reload the page to restore it.')});window.munks={getState:()=>({running,splitScreen:split,activities:activityMissions(state).map(m=>({name:m.short,done:m.done,progress:m.progress})),missions:missions(state).map(m=>({name:m.short,done:m.done,progress:m.progress})),players:state.players.map((p,i)=>({name:i?'Momo':'Pip',x:p.x,y:p.y,z:p.z}))}),restart};
