...
export function createState(){return {players:[{x:-2,z:12,y:0,vy:0,angle:0,walk:0,ook:0,air:false,bump:0,bumpCooldown:0,land:0,cheer:0,hello:0,idle:0,moving:false,reaction:null,lastReaction:null},{x:2,z:12,y:0,vy:0,angle:0,walk:0,ook:0,air:false,bump:0,bumpCooldown:0,land:0,cheer:0,hello:0,idle:0,moving:false,reaction:null,lastReaction:null}],time:0,reactionSeed:0x6d756e6b,birds:Array.from({length:7},(_,i)=>({x:-12+i*4,z:10+i%2*6,phase:i*.87,scatter:null,cooldown:0})),bells:[false,false],bellUntil:[0,0],bellPlayers:[-1,-1],band:0,bandPlayers:[],songComplete:false,foam:0,bubbles:false,pins:TARGETS.map(t=>({...t,down:false})),balls:[{x:26,z:32,vx:0,vz:0},{x:29,z:32,vx:0,vz:0},{x:32,z:32,vx:0,vz:0}],hoops:[false,false,false],bouncers:[],events:[],celebrated:false,solid:[]}}
...
export function interact(s,i){const p=s.players[i];const ladder=LADDERS.find(l=>dist(p,l)<3);if(ladder){if(p.y<3){p.y=7;p.z=-25;p.vy=0;p.air=false;emit(s,'Up in the canopy! Ring a bell with E / Enter.')}else{p.y=0;p.z=-19;p.vy=0;p.air=false;emit(s,'Back on the ground.')}return}
...
const a=s.players[0],b=s.players[1],gap=dist(a,b);if(gap<1.04&&Math.abs(a.y-b.y)<1.5){const nx=gap>.001?(b.x-a.x)/gap:1,nz=gap>.001?(b.z-a.z)/gap:0,push=(1.04-gap)/2;for(const [p,sign] of [[a,-1],[b,1]]){const x=p.x+nx*push*sign,z=p.z+nz*push*sign;if(!s.solid.some(o=>p.y<o.h&&Math.hypot(x-o.x,z-o.z)<o.r+.48)){p.x=Math.max(-46,Math.min(46,x));p.z=Math.max(-43,Math.min(43,z));}}if((a.moving||b.moving)&&a.bumpCooldown<=0&&b.bumpCooldown<=0){for(const p of [a,b]){p.bump=.85;p.bumpCooldown=1.2;p.hello=1.6;}emit(s,'Boop!','bonk',(a.x+b.x)/2,(a.z+b.z)/2)}}
